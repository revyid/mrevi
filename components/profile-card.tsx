...
// Honest minimal fallback — no invented job titles.
const DEFAULT_ROLES = ["Developer"];
...
export function ProfileCard({ settings = {} }: ProfileCardProps) {
  ...
  // Only render icons for links that are actually configured (skips "" and "#").
  const socialLinks = [
    { key: "facebook",  href: social.facebook,  label: "Facebook" },
    { key: "twitter",   href: social.twitter,   label: "Twitter" },
  ...
  // Initials for the avatar-less state (keeps the layout stable).
  const initials = name
    .split(/\s+/)
    .filter(Boolean)
    .slice(0, 2)
      ...
      <div className="flex justify-center px-6 pt-6 lg:pt-5">
        <div className="w-[150px] h-[150px] sm:w-[220px] sm:h-[220px] lg:w-[clamp(130px,22vh,220px)] lg:h-[clamp(130px,22vh,220px)] rounded-2xl overflow-hidden">
          {avatar ? (
            ...
            <div className="w-full h-full flex items-center justify-center bg-muted text-muted-foreground text-4xl font-bold font-heading">
              {initials || "—"}
            </div>
          )}
        </div>
          ...
          <div className="flex items-center justify-center gap-2">
            {socialLinks.map((s) => (
              <SocialIcon key={s.key} href={s.href} label={s.label}>
                {SOCIAL_ICONS[s.key as keyof typeof SOCIAL_ICONS]}
              </SocialIcon>
            ))}
          </div>
        )}
